  ...
  return (
    <div className="flex flex-col h-full justify-between">
      ...
      <div>
        ...
        {/* Section Label */}
        <p className="px-3 mb-2 text-[10px] font-semibold tracking-[0.15em] text-zinc-600 uppercase">
          Navigation
        </p>
            ...
            return (
              <button
                ...
                className={`w-full flex items-center gap-3 px-3.5 py-2.5 rounded-xl text-sm font-medium transition-all duration-200 group ${
                  isActive
                    ? "bg-amber-500/10 text-amber-400 border border-amber-500/20 shadow-[inset_0_1px_0_rgba(251,191,36,0.05)]"
      ...
      <div className="pt-4 border-t border-zinc-800/60 mt-auto">
        <button
          onClick={onSignOut}
          className="w-full flex items-center gap-3 px-3.5 py-2.5 rounded-xl text-sm font-medium text-zinc-600 hover:text-red-400 hover:bg-red-500/5 border border-transparent hover:border-red-500/10 transition-all duration-200 group"
        >
          <LogOut className="h-4 w-4 shrink-0 text-zinc-600 group-hover:text-red-400 transition-colors duration-200" strokeWidth={1.75} />
          Sign Out
        </button>
      </div>
    </div>
